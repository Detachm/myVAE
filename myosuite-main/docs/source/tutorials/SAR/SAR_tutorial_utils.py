...
def get_vid(name, env_name, seed, episodes, video_name, determ=False,
    ...
    for i,__ in tqdm(enumerate(range(episodes))):
        ...
        while not done:
            o = vec.normalize_obs(env.unwrapped.get_obs())
...
def SAR_RL(env_name, policy_name, timesteps, seed, ica=None, pca=None, normalizer=None, phi=None, syn_nosyn=False):
    """
    Trains a SAR-RL agent on the specified environment.
    """
    log_dir = f'{policy_name}_results_{env_name}_{seed}'
    ...
    # 添加回调函数来保存最佳模型
    callback = SaveSuccesses(check_freq=1000, log_dir=log_dir, env_name=env_name)
